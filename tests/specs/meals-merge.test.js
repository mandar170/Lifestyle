...
async function run(page) {
  ...
  const out = await page.evaluate(async () => {
    ...
    // 2) Coexistence: quick-adding a planned meal creates a logged row alongside it.
    S.meals.length = 0;
  ...
  t.eq('quick-add keeps plan + adds logged (coexistence)', out.breakfastRowCount, 2);
  t.eq('both statuses present', out.breakfastStatuses, 'logged,planned');
